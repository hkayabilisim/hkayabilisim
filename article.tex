...
%Alignment evaluation scores should not be used to reach a quick decision about the ``goodness'' of alignment. An alignment result may have a lower score but the result may not be acceptable as in the Figure~\ref{Figure10} where DTW generated alignment has the lowest score but suffers from overfitting. The peaks are forced to align at the expense of a visible distortion. Therefore trusting a single score in judging the quality of alignment is not advised. A better solution would be to observe the effects of different alignments in later stages. For instance, different techniques can be used to align DNA sequences and their performances can be evaluated in terms of mutation screening results.
